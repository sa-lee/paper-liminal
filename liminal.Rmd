...
author:
  - name: Stuart Lee
    ...
    address: |
      ...
      | Monash University
    email: \email{stuart.andrew.lee@gmail.com}
  - name: Ursula Laa 
    affiliation: Monash University
    address: |
      | Department of Econometrics and Business Statistics,
...
workflow. @Brehmer2014-hk characterized two main workflow steps that an
...
techniques. @Buja2008-fn used tours to guide analysts during the optimization
...
\pkg{liminal} and plans for extensions to the software. Throughout the paper
...
freedom [@Maaten2008-sk]. There are several subtleties of the algorithm that 
...
neighborhood graph of the $i$th observation [@Van_Der_Maaten2014-zn].
...
LargeVis [@Tang2016-oz], and like t-SNE acts on the k-nearest neighbor graph.
...
subspace of $d$-dimensional projections.  In practice, we first form  our data into a sphere via principal components to reduce dimensionality of $X$ prior to running the tour. Instead of picking projections at  random, a *guided tour* can be used to generate a sequence of 'interesting' 
projections as quantified by an index function [@Cook1995-bi]. While our software,\pkg{liminal} is able to visualize guided tours, our focus in the case studies uses the grand tour to see global structure in the data. 
...
visualizations, drawn with the \pkg{Vega-Lite} grammar of interactive graphics via
...
(described by @Amezquita2020-at). We performed quality control using the **scater** package
...
get a sense of the shape of the clusters as seen from the linked video in Figure \@ref(fig:mouse-01). If one was interested in performing more in-depth cluster analysis, we recommend an iterative approach of removing large clusters and then re-running the \pkg{liminal} view as a way finding more granular cluster structure. One could perform this approach manually via the \pkg{liminal} interface by returning the regions identified by brushing on the tour or embedding view. 
