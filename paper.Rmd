...
pairwise distance in the original high-dimensional space [@Ovchinnikova2020-sy].
...
should be close according to the estimated pairwise distance. 
...
an embedding  $Y$ such that the pairwise distances (inner products) in $Y$ 
approximate the pairwise distances (inner products) in $X$ 
...
to estimate the pairwise geodesic distance of points in $X$ then uses classical 
MDS to construct $Y$ [@Silva2003-xw]. Other approaches are based on diffusion
processes such asdiffusion maps [@Coifman2005-ak]. A recent example of this 
approach is the PHATE algorithm [@Moon2019-ce]. Here an affinity matrix is 
estimated via the pairwise distance matrix and k-nearest neighbours graph of $X$. 
...
of brushing acts directly on $k$-nearest neighbors ($l$-nn) graph computed from a
...
neighboring points, or by highlighting. Brush compostion, such as 'and', 'or', or 'not' brushes,  could also be used to interrogate mismatches between the $k$-nn graphs estimated from both the embedding and tour views. This is left for 
...
a animated scatterplot. This process of constructing bases and intermediate 
frames and visualising the resulting projection is akin to making a "flipbook" 
animation. Like with a flipbook, an interface to the tour requires the ability
...
view (the special case of $k=1$ nearest neighbors brush). In future
...
is known with varying success.  

In the second case study, we explore a well-clustered single cell
transcriptomics data of mice retina... 
...
aboutthe  t-SNE algorithm. Our first data set consists of five spherical 5-$d$
...
<embedded video liminal gaussian>
...
the tSNE layout. This can be seen from our brushing action on the tSNE side. 

<embedded video liminal hierachcial >

We repeat the process of simulating Gaussian clusters in video <>, but this time
we have a cluster that has more granular structure. This time we have two
Gaussian clusters with equal covariance embedded in $10-d$, and a third cluster
with hierachcial structure. This cluster has two 3-$d$ clusters embedded in 
...
Again tSNE has correctly identified the sub-clusters but their relative 
...
they become more apparent. When we brush the sub-clusters in the tSNE, their
relative placement is again exaggerated, with the tour showing that they
are indeed much closer than the impression the tSNE view gives.
...
(ref:fake-trees) Example high-dimensional tree shaped data, $n = 3000$ and $p = 100$. **(a)** The true data lies 2-d tree like structure consisting of ten branches. This data is available in the `phateR` package and is simulated via diffusion-limited aggregation (a random walk along the branches of the tree) with Gaussian noise added [@Moon2019-ce]. **(b)** The first two principal components, which form the initial projection for the tour, note that the backbone of the three is obscured by this view. **(c)** The default tSNE view breaks the global structure of the tree. **(d)** Altering the initialision of tSNE using the first two principal components, clusters the tree at its branching points.
...
To check and verify the clustering we construct a liminal view. Because most off the variability in
expression is explained by the first 5 PCs, we tour those. The t-SNE view
is computed from all PCs. 

- compare to just using the HVGs themselves with the tour
- neighbourhood based brushing to check how diffuse or distorted clusters are
...
- limitations: large $n$, latency issues
- future work: regularisation tour, slicing and sectioning
