...
an embedding  $Y$ such that the pariwise distances (inner products) in $Y$ 
approximate the pairwise distances (inner products) in $X$ 
[@Torgerson1952-am; @Kruskal1964-cz]. There are many 
variants of MDS, such as nonmetric scaling which amounts to replacing
...
estimated via the pairwise distance matrix and k-nearest neighbors graph of $X$. 
The algorithm denoises estimated distances in high-dimensional space via 
...
views fromembedding methods and allows them to see how the embedding method 
alters theglobal structure of their data. Using ensembles allows the use of 
interaction techniques, that allow analysts to perform cluster orientation 
tasks vialinking multiple views. This approach allows our interface, to achieve 
...
by removing cells with high proportion of mitocondrial gene expression and low
...
Using the PCs we built a shared nearest neighours graph (with $k = 10$) and 
used Louvain clustering to generate clusters. To check and verify the 
clustering we construct a liminal view. Because most off the variablity in
...
[@quickdraw]. The data are sketches drawn by people of common concepts
