...
view (the special case of $k=1$ nearest neighbors brush). It is possible
to shift to one-to-many linking by increasing $k$ via the 'controls'
panel of the interface.  If desired, the distance
metric used to estimate nearest neighbours can by chosen by the user. 
...
covariance matrix. We then computed a t-SNE layout using default settings,
...
spatial and linked brushing approaches to look at neighbourhood preservation
(which can be seen via the tour) and to look at the affect of applying different
feature selection techniques and distance metrics.
...
used Louvain clustering to generate clusters. To check and verify the 
clustering we construct a liminal view. Because most off the variability in
