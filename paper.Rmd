...
and real data. In the first case study, we step through three simulated datasets
to show the `liminal` interface. The first simulation contains 3 clusters
sampled from a 5-d gaussian distribution that are well embedded by t-SNE.
The second is another example of spherical clusters but this time, they are
tightly clustered and are distorted by t-SNE, which is immediately apparent in 
the tour view. In the final simulation we
look at a tree structure (similar to trajectories in single cell 
