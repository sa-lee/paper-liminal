...
ground truth.
\textcolor{red}{Ursula: maybe work out the "dimension reduction" examples in a
bit more detail? They are difficult to understand for someone who does not know
about them already (or at least for me), and it wasn't very clear why clustering
is not part of that (I guess because we want to actually tag clusters for further
...
\textcolor{red}{Ursula: suggestion for the structure of the following paragraphs:
start with "no one-size fits all", then talk about interactive interfaces to assess
embeddings, and introduce tours when talking about existing interfaces, and finish by
talking about liminal approach.}
...
<!--
Ursula: I don't think this paragraph is needed.
...
and classical MDS is  used to generate $Y$.
-->


A general difficulty with using 
non-linear DR methods for exploratory data analysis is selecting and tuning 
appropriate parameters. For concretenes here we focus on t-distributed
...
\textcolor{red}{Ursula: is there an intuitive explanation why we want to tune
perplexity rather than working with sigma directly?}
...
\textcolor{red}{Ursula: here you are following the ordering from the cited
paper, but maybe it makes sense to discuss making comparisons before
talking about posing queries?}

<!--
Ursula: I think for these subsections we need to have a first paragraph
that specifies what the concept means and why it is important here,
before describing how it is incorporated in liminal
-->
...
\textcolor{red}{Interesting projections or interesting points?}
The
domain of the axis scales from running a tour is called the half range, and is 
computed by rescaling the input data onto hyper-dimensional
\textcolor{red}{d-dimensional?}
unit cube. We bind 
...
to make meaniningful comparisons. In \pkg{liminal} the embedding
...
\textcolor{red}{Ursula: can I use restart to replay the same tour path (i.e. find a
previously seen projection), or will it start a new random selection of bases?}
...
(ref:liminal-hierarchical) Screenshots of the \pkg{liminal} interface applied to sub-clustered data, a video of the tour animation is available at https://player.vimeo.com/video/43963590.
...
@Macosko2015-ot using the \pkg{scRNAseq} Bioconductor package [@scRNAseq-d]. We
have followed a standard workflow for pre-processing and normalizing this data 
(described by @Amezquita2020-at): we performed QC using the \pkg{scater} package
...
values and  finally selected highly variable genes (HVGs) using \pkg{scran} 
...
implementing this brush in \pkg{liminal},  using the  \pkg{FNN} or \pkg{RcppAnnoy}
